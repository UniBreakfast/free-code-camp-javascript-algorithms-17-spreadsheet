...
const range = (start, end) => Array(end - start + 1)
  .fill(start).map((element, index) => element + index);
...
const evalFormula = (x, cells) => {
  ...
  const addCharacters = character1 => character2 => 
    num => charRange(character1, character2).map(elemValue(num));
  const rangeExpanded = x.replace(
    rangeRegex, 
    (_match, char1, num1, char2, num2) => rangeFromString(num1, num2)
  ...
  return functionExpanded === x ? 
    functionExpanded : evalFormula(functionExpanded, cells);
}
